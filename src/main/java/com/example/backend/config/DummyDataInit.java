 ...
 * category별로 시정소식(NOTICE) / 정보공개(INFO) / 시민참여(PARTICIPATE)를 분리한다.
 */
@Slf4j
@Profile({"local", "dev"})
@Component
    ...
    public void run(ApplicationArguments args) {
        ...
        List<PortalItem> items = List.of(
                ...
                PortalItem.builder()
                        ...
                        .category(PortalCategory.NOTICE)
                ...
                PortalItem.builder()
                        .type(ItemType.NOTICE)
                        .category(PortalCategory.INFO)
                        .title("[공개] 행복시 공공건축물 내진성능평가 결과 공개")
                ...
                PortalItem.builder()
                        .type(ItemType.NOTICE)
                        .category(PortalCategory.PARTICIPATE)
                        .title("[공모] 시민과 함께하는 2026 행복시 주민참여예산 공모")
                ...
                PortalItem.builder()
                        .type(ItemType.NOTICE)
                        .category(PortalCategory.PARTICIPATE)
                        .title("[안내] 행복시 민원여권과 여권 발급 대기시간 안내")
        ...
        log.info("[DummyDataInit] 행복시청 공지 더미 데이터 {}건 초기화 완료 (NOTICE/INFO/PARTICIPATE 분리)", items.size());
    }
}
